...
\begin{tikzpicture}[x=1cm,y=1cm,font=\small\sffamily]
  \def\R{2.0} % Reduced slightly from 2.2
  \def\r{1.0} 
  \def\cx{0}
  \def\cy{0}
  ...
  \begin{scope}[shift={(72:0.3)}]
    ...
    \node[color=white, font=\bfseries] at (72:\R*0.65) {40\%};
  \end{scope}
  ...
  % Legend with more padding
  \def\lx{3.2} \def\ly{0.8} \def\sw{0.4} \def\sh{0.3}
  \foreach \color/\label/\dy in {GoldHighlight/Problem-Driven/0, Charcoal!80/Deadline-Driven/0.7, Charcoal!60/Failure-Driven/1.4, Charcoal!40/Experimentation/2.1, Charcoal!20/Community/2.8} {
...
\begin{tikzpicture}[x=1.3cm,y=1.3cm,>=Stealth]
  % Professional Loop Design - Space Optimized
  \tikzset{loopnode/.style={circle, draw=Charcoal, line width=2.5pt, minimum size=3.4cm, align=center, fill=white, font=\Large\bfseries\Charcoal, drop shadow={shadow xshift=3pt, shadow yshift=-3pt}}}
  
  % Increased circle radius to prevent overlap
  \node[loopnode, draw=GoldHighlight] (need) at (90:4.6) {NEED};
  \node[loopnode] (build) at (210:4.6) {BUILD};
  \node[loopnode] (validate) at (330:4.6) {VALIDATE};

  % detailed icons - repositioned
  \begin{scope}[shift={(need.south)}, yshift=0.9cm, scale=0.6]
    \fill[GoldHighlight!15] (0,0.4) circle (0.5);
    \draw[GoldHighlight, line width=1.5pt] (0,0.4) circle (0.3);
    \draw[GoldHighlight, line width=1.5pt] (-0.1,0.1) -- (0.1,0.1);
  \end{scope}
  
  \begin{scope}[shift={(build.south)}, yshift=0.9cm, scale=0.6, rotate=-45]
    \fill[Charcoal!80] (-0.08,0) rectangle (0.08,0.7);
    \fill[Charcoal] (-0.25,0.6) rectangle (0.25,0.85);
  \end{scope}

  \begin{scope}[shift={(validate.south)}, yshift=0.9cm, scale=0.6]
    \draw[SteelBlue, line width=2.5pt] (-0.2,0.2) -- (0,0) -- (0.3,0.4);
  \end{scope}

  % 3D-style Curved Arrows - labels shifted to prevent overlap
  \tikzset{flowarrow/.style={->, line width=5pt, draw=Charcoal!90, double=white, double distance=1.5pt}}
  
  \draw[flowarrow] (need) to[bend right=40] node[midway, left=25pt, font=\bfseries\sffamily\SlateGrey, rotate=60] {Problem Emergence} (build);
  \draw[flowarrow] (build) to[bend right=40] node[midway, below=25pt, font=\bfseries\sffamily\SlateGrey] {System Implementation} (validate);
  \draw[flowarrow] (validate) to[bend right=40] node[midway, right=25pt, font=\bfseries\sffamily\SlateGrey, rotate=-60] {Functional Feedback} (need);

  % Central Graphic - smaller to provide room
  \draw[Charcoal!10, line width=12pt] (0,0) circle (2.0cm);
  \node[font=\huge\bfseries\Charcoal] at (0,0.2) {CORE};
  \node[font=\Large\bfseries\GoldHighlight] at (0,-0.6) {PROCESS};

  % Details
  \node[font=\footnotesize\bfseries\GoldHighlight] at (90:2.9) {Trigger};
  \node[font=\footnotesize\bfseries\Charcoal] at (210:2.9) {Execution};
  \node[font=\footnotesize\bfseries\Charcoal] at (330:2.9) {Feedback};
\end{tikzpicture}%
}}%
\else
