...
\begin{tikzpicture}[x=1cm,y=1cm,font=\small\sffamily]
  ...
  % Legend with generous spacing to prevent overlap
  \def\lx{3.8} \def\ly{1.0} \def\sw{0.4} \def\sh{0.3}
...
\begin{tikzpicture}[x=1.3cm,y=1.3cm,>=Stealth]
  % Ultimate 3D/Elite Loop Design - Spacing Fixed
  ...
  \node[loopnode, draw=GoldHighlight] (need) at (90:4.2) {NEED};
  \node[loopnode] (build) at (210:4.2) {BUILD};
  \node[loopnode] (validate) at (330:4.2) {VALIDATE};

  % Icons (Optimized Spacing)
  \begin{scope}[shift={(need.center)}, yshift=-0.4cm, scale=0.7]
    \fill[GoldHighlight!15, even odd rule] (0,0.4) circle (0.6); 
  ...
  \begin{scope}[shift={(build.center)}, yshift=-0.4cm, scale=0.7, rotate=-45]
    \fill[Charcoal!80] (-0.08,0) rectangle (0.08,0.7); 
    \fill[Charcoal] (-0.3,0.6) rectangle (0.3,0.9);
  \end{scope}

  \begin{scope}[shift={(validate.center)}, yshift=-0.4cm, scale=0.7]
    \draw[SteelBlue, line width=3pt, line cap=round] (-0.3,0.2) -- (-0.05,-0.1) -- (0.4,0.5);
  \end{scope}

  % 3D-style Curved Arrows with Overlay Labels (Clean separation)
  \tikzset{flowarrow/.style={->, line width=6pt, draw=Charcoal!90, double=white, double distance=2pt}}
  \tikzset{flowlabel/.style={midway, font=\bfseries\sffamily\small, fill=white, inner sep=2pt, rounded corners=2pt, text=SlateGrey}}
  
  \draw[flowarrow] (need) to[bend right=40] node[flowlabel, left=20pt, rotate=60] {Problem Emergence} (build);
  \draw[flowarrow] (build) to[bend right=40] node[flowlabel, below=20pt] {System Implementation} (validate);
  \draw[flowarrow] (validate) to[bend right=40] node[flowlabel, right=20pt, rotate=-60] {Functional Feedback} (need);

  % Central Graphic (Separated)
  \draw[Charcoal!10, line width=15pt] (0,0) circle (2.1cm);
  \node[font=\huge\bfseries\Charcoal] at (0,0.3) {CORE};
  \node[font=\Large\bfseries\GoldHighlight] at (0,-0.4) {LOOP};

  % Strategy Sub-notes
  \node[font=\footnotesize\bfseries\GoldHighlight, align=center] at (90:2.6) {Technical\\Catalyst};
  \node[font=\footnotesize\bfseries\Charcoal, align=center] at (210:2.6) {Practical\\Execution};
  \node[font=\footnotesize\bfseries\Charcoal, align=center] at (330:2.6) {Real-world\\Check};
\end{tikzpicture}%
}}%
\else
